...
\title{Glauber Dynamics via Tensor Trains}
\author{Anthony Yoon}
\date{May 2026}
...
To provide some intuition for $N$ spin chains, consider a singular electron spinning. We make the assumption that there exists no magnetic field within the system, which means that there is no bias towards any of the states. Then, let $\alpha /2 $ denote the rate per unit time at which the particle makes a transition in between each state. We can see that the time evolution of the probability that the spin takes on a value $\sigma$ at time $t$ can be expressed as
\[
...
&= -\alpha q(t)
\end{align*}
The solution to the above differential equation is given as $q(t) = q(0)e^{-\alpha t}$, which implies that the mean spin decays exponentially with relaxation time $\alpha^{-1}$ (equivalently, at rate $\alpha$).

Now, consider the spin system of size $N$ as previously mentioned. We make the assumption that for a spin $i$, its neighbors' momentum influences the spin of the system. Note that there are $2^N$ elements in the sample space of the spin configuration. Let $w_i(\sigma_i)$ denote the probability per unit time that spin $i$ flips while the others remain momentarily fixed. The derivative with respect to time of the probability function $p(\sigma_1, \sigma_2, \dots, \sigma_N, t)$ can be expressed as
\[
...
\end{align*}
We prove the two equalities directly. Write $\theta := \frac{J}{kT}(\sigma_{i-1} + \sigma_{i+1})$, so that the exponent equals $\pm\frac{J}{kT}\sigma_i(\sigma_{i-1} + \sigma_{i+1}) = \sigma_i(\pm\theta)$.

For the first equality, recall the defining relations $e^{\pm\theta} = \cosh\theta \pm \sinh\theta$. Since $\sigma_i \in \{+1, -1\}$, the quantity $\cosh\theta + \sigma_i\sinh\theta$ equals $\cosh\theta + \sinh\theta = e^{\theta}$ when $\sigma_i = +1$ and $\cosh\theta - \sinh\theta = e^{-\theta}$ when $\sigma_i = -1$; in both cases it is $e^{\sigma_i\theta}$. Hence $e^{\sigma_i\phi} = \cosh\phi + \sigma_i\sinh\phi$ for every real $\phi$, and taking $\phi = \pm\theta$ (with $\cosh$ even and $\sinh$ odd) gives
\[
\exp\!\left[\pm\tfrac{J}{kT}\sigma_i(\sigma_{i-1} + \sigma_{i+1})\right]
= e^{\sigma_i(\pm\theta)}
= \cosh\theta \pm \sigma_i\sinh\theta,
\]
which is the first line.

For the second equality, factor the nonzero $\cosh\theta$ out of the first line,
\[
\cosh\theta \pm \sigma_i\sinh\theta = \cosh\theta\left(1 \pm \sigma_i\tanh\theta\right),
\]
and verify $\tanh\theta = \tfrac{1}{2}(\sigma_{i-1} + \sigma_{i+1})\tanh\tfrac{2J}{kT}$ by direct case check. As $\sigma_{i-1}, \sigma_{i+1} \in \{\pm 1\}$, the sum $\sigma_{i-1} + \sigma_{i+1}$ is one of $-2, 0, 2$: if it is $0$ then $\theta = 0$ and both sides vanish; if it is $2$ then $\theta = \tfrac{2J}{kT}$ and $\tanh\theta = \tanh\tfrac{2J}{kT} = \tfrac{1}{2}(2)\tanh\tfrac{2J}{kT}$; if it is $-2$ then $\theta = -\tfrac{2J}{kT}$ and $\tanh\theta = -\tanh\tfrac{2J}{kT} = \tfrac{1}{2}(-2)\tanh\tfrac{2J}{kT}$. Substituting $\sigma_i\tanh\theta = \tfrac{1}{2}\sigma_i(\sigma_{i-1} + \sigma_{i+1})\tanh\tfrac{2J}{kT}$ yields
\[
\cosh\theta\left(1 \pm \sigma_i\tanh\theta\right)
= \cosh\theta\left(1 \pm \tfrac{1}{2}\sigma_i(\sigma_{i-1} + \sigma_{i+1})\tanh\tfrac{2J}{kT}\right),
\]
...
\emph{Notation.} A \emph{configuration} $\sigma = (\sigma_1, \dots, \sigma_N) \in \{\pm 1\}^N$ is a full assignment of spins to the $N$ sites. A \emph{subscript} $\sigma_i$ denotes the spin at \emph{site} $i$; a \emph{superscript} $\sigma^i$ denotes the configuration obtained from $\sigma$ by flipping that one site (so $(\sigma^i)^i = \sigma$). The rate $w_i(\sigma)$ is the probability per unit time of flipping site $i$ given the whole configuration $\sigma$: its subscript is a site, its argument a configuration. The vector $\brho \in \R^{2^N}$ carries one component per configuration, $[\brho]_\sigma = \rho(\sigma)$, and the generator $L \in \R^{2^N \times 2^N}$ is indexed by configurations, $L_{\sigma\sigma'}$ being the entry in row $\sigma$ and column $\sigma'$. To display $L$ as an explicit array we fix, once and for all, an arbitrary enumeration $\sigma^{(1)}, \dots, \sigma^{(2^N)}$ of the $2^N$ configurations and label the rows and columns by it; the parenthesised superscript $\sigma^{(k)}$ is an \emph{enumeration index} for the $k$-th configuration, and is not to be confused with the site subscript $\sigma_i$ or the single-flip superscript $\sigma^i$.
...
\underbrace{
\begin{bmatrix}
-\displaystyle\sum_i w_i(\sigma^{(1)}) & \ell_{\sigma^{(1)} \sigma^{(2)}} & \ell_{\sigma^{(1)} \sigma^{(3)}} & \cdots & \ell_{\sigma^{(1)} \sigma^{(2^N)}} \\[10pt]
\ell_{\sigma^{(2)} \sigma^{(1)}} & -\displaystyle\sum_i w_i(\sigma^{(2)}) & \ell_{\sigma^{(2)} \sigma^{(3)}} & \cdots & \ell_{\sigma^{(2)} \sigma^{(2^N)}} \\[10pt]
\ell_{\sigma^{(3)} \sigma^{(1)}} & \ell_{\sigma^{(3)} \sigma^{(2)}} & -\displaystyle\sum_i w_i(\sigma^{(3)}) & \cdots & \ell_{\sigma^{(3)} \sigma^{(2^N)}} \\[6pt]
\vdots & \vdots & \vdots & \ddots & \vdots \\[4pt]
\ell_{\sigma^{(2^N)} \sigma^{(1)}} & \ell_{\sigma^{(2^N)} \sigma^{(2)}} & \ell_{\sigma^{(2^N)} \sigma^{(3)}} & \cdots & -\displaystyle\sum_i w_i(\sigma^{(2^N)})
\end{bmatrix}
}_{L}
\begin{bmatrix}
\rho(\sigma^{(1)}, t) \\[8pt]
...
\emph{Uniqueness.} Every single-flip rate is strictly positive: from Section~\ref{sec:glauber}, $w_i(\sigma') \in \{\tfrac{\alpha}{2},\, \tfrac{\alpha}{2}(1-\gamma),\, \tfrac{\alpha}{2}(1+\gamma)\}$, all positive since $\alpha > 0$ and $|\gamma| < 1$. To see that $L$ is \emph{irreducible} --- that from any configuration the chain can reach any other through a sequence of transitions of positive rate --- fix two configurations $\sigma, \tau \in \{\pm 1\}^N$ and let $S = \{\, i : \sigma_i \neq \tau_i \,\}$ be the set of sites at which they disagree. Flip those sites one at a time, in any fixed order $i_1, \dots, i_m$ with $m = |S|$, producing the sequence
\[
\sigma = c_0 \;\to\; c_1 \;\to\; \cdots \;\to\; c_m = \tau,
\qquad c_k = (c_{k-1})^{i_k}.
\]
Each step changes exactly one spin, so the transition $c_{k-1} \to c_k$ has off-diagonal entry $L_{c_k\, c_{k-1}} = w_{i_k}(c_{k-1}) > 0$ and is therefore allowed. Hence $\sigma$ reaches $\tau$; as $\sigma$ and $\tau$ were arbitrary, $L$ is irreducible. Equivalently, the state space $\{\pm 1\}^N$ is the vertex set of the $N$-dimensional hypercube whose edges are exactly the single-flip transitions, and that graph is connected. By the Perron--Frobenius theorem for irreducible generators the eigenvalue $0$ is simple, so $\dim\ker L = 1$ and the stationary vector is unique up to scale. The corresponding Perron eigenvector may moreover be chosen with strictly positive entries, so it can be rescaled to satisfy $\|\brho\|_1 = 1$, giving a genuine probability distribution.
\end{proof}
...
Here each nonzero entry of a core acts as a transition of a finite-state machine scanning the chain: it \emph{emits} (places) that entry's local operator onto the current site while advancing the bond index from one state to the next, and a two-site coupling is assembled by emitting one factor and later \emph{closing} it with the matching factor at the partner site, the bond state in between remembering the still-owed factor. Reading $L$ as a finite-state machine --- start; or emit $Z$ and wait to emit $G$; or emit $G$ and wait to emit $Z$; or done --- gives, for the \emph{open} chain, the exact rank-$4$ MPO with bulk core
\[
W^{[i]} =
\begin{pmatrix}
I  & 0   & 0   & 0 \\
...
\begin{remark}[The order-$4$ core as an order-$3$ MPS core]\label{rem:mpo-as-mps}
Each core $W^{[i]} \in \R^{R_{k-1}\times n \times n \times R_k}$ above is an order-$4$ tensor: two bond legs $R_{k-1}, R_k$ (the outer $4\times4$ or $6\times6$ layout) and two physical legs $n \times n$ (the row/column indices $(\sigma_i, \sigma_i')$ of each inner $2\times2$ block). Fusing the two physical legs into a single super-index of dimension $n^2$,
\[
W^{[i]} \in \R^{R_{k-1}\times n \times n \times R_k}
\;\longmapsto\;
\widetilde W^{[i]} \in \R^{R_{k-1}\times n^2 \times R_k},
\qquad (\sigma_i, \sigma_i') \mapsto \operatorname{vec}(\,\cdot\,),
\]
turns each order-$4$ MPO core into an order-$3$ MPS core. This is the operator-as-state (vectorisation) view: the generator $L$ on $\R^{n^N}$ is reread as a single MPS vector $\operatorname{vec}(L) \in \R^{(n^2)^N}$ on super-sites of dimension $n^2 = 4$, whose cores carry $\operatorname{vec}(I), \operatorname{vec}(G), \operatorname{vec}(Z), \operatorname{vec}(aS), \dots$ in the fused slot. The reshape is lossless and leaves the bond dimensions untouched ($4$ open, $6$ on the ring), so it is purely a relabelling; an order-$3$ core is possible only through this physical-leg fusion, since a genuine operator core needs both a row (output) and a column (input) index and is therefore intrinsically order $4$.
\end{remark}
...
We close with a naive cost comparison of the two solvers. Constants are dropped, the physical dimension $n = 2$ is treated as a constant, and the working TT bond dimension of the iterate is held at $r$. Let $N$ be the chain length, $R$ the MPO bond dimension of $L$ (here $R = 4$ open, $R = 6$ on the ring), and $m$ the GMRES Krylov/restart depth. The one structural fact underlying both estimates is that each method visits the $N$ sites and pays a cost \emph{polynomial in the ranks} rather than exponential in $N$: the dense state space is $2^N$, but neither algorithm ever forms a length-$2^N$ vector. Both costs are therefore linear in $N$ --- the entire point of the tensor-train format.

\paragraph{DMRG (variational null vector of $M = L^\top L$).}
The work is one two-site problem per site, repeated over sweeps. The two-site supercore has $r\,n\,n\,r = O(r^2)$ entries, and DMRG runs on $M = L^\top L$, whose MPO bond dimension is $R^2$. Assembling and solving the dense two-site effective operator therefore costs $\bigO{r^4 (R^2)^2} = \bigO{r^4 R^4}$ per site, giving
\[
T_{\mathrm{DMRG}} = \bigO{N_{\mathrm{sweeps}} \cdot N \cdot r^4 R^4}.
\]
The cost is quartic in the rank $r$ and carries a factor $R^4$ (that is, $1296$ on the ring): forming $L^\top L$ \emph{squares} the MPO bond ($R \to R^2$), and the two-site assembly squares it once more. The same squaring raises the condition number, $\kappa(M) = \kappa(L)^2$, so $N_{\mathrm{sweeps}}$ also tends to grow --- a second, implicit penalty.

\paragraph{Bordered TT-GMRES ($(L + \mathbf{1}\mathbf{1}^\top)\brho = \mathbf{1}$).}
The work is $T$ restart cycles, each performing $m$ Arnoldi steps. Per Arnoldi step the matrix-vector product $Av$ raises the rank $r \to Rr$ at cost $\bigO{N R^2 r^2}$, after which the result is TT-rounded back to rank $r$ at cost $\bigO{N (Rr)^3} = \bigO{N R^3 r^3}$, the dominant term; the modified Gram--Schmidt orthogonalisation accumulates to $\bigO{m^2 N r^3}$ over a cycle. With $m R^3 \gg m^2$ the apply-and-round term wins, so
\[
T_{\mathrm{GMRES}} = \bigO{T \cdot m \cdot N \cdot r^3 R^3}.
\]
This is one power lower than DMRG in \emph{both} the rank $r$ and the MPO bond $R$, precisely because GMRES never forms $L^\top L$ and so pays $R$ rather than $R^2$. The price is the extra $T\,m$ matrix-vector factor, the loss of the symmetric variational guarantee, and --- since this is inexact (truncated) GMRES --- a hard residual floor set by \texttt{max\_rank}: too small an $r$ stagnates above the tolerance no matter how large $T$ and $m$ are.

\begin{table}[H]
\centering
\begin{tabular}{l c c c c}
\hline
Method & per site/step & total (naive) & rank & bond \\
\hline
DMRG            & $r^4 R^4$ & $\bigO{N_{\mathrm{sweeps}}\, N\, r^4 R^4}$ & $r^4$ & $R^4$ \\
Bordered TT-GMRES & $r^3 R^3$ & $\bigO{T\, m\, N\, r^3 R^3}$              & $r^3$ & $R^3$ \\
\hline
\end{tabular}
\caption{Naive per-step and total cost of the two solvers. Both scale linearly in $N$. DMRG is a full power of the rank $r$ and of the MPO bond $R$ more expensive per site --- the concrete price of the $L^\top L$ squaring --- but is a monotone variational solve whose residual is always reducible by adding rank. TT-GMRES is cheaper per step and sidesteps the squaring, at the cost of $T\,m$ matrix-vector products, non-symmetric convergence that depends on conditioning, and a rank-imposed residual floor. This is the quantitative form of the trade-off noted in Section~\ref{sec:recovery}.}
